...
class MusicLibraryController 
  ...
  def list_artists 
    Artist.all.uniq.sort_by(&:name).each.with_index(1) {|art, index| puts "#{index}. #{art.name}"}
  end
  ...
  def play_song
    ...
    selected_song = Song.all.sort_by(&:name)[input-1]
    if selected_song != nil && (1..Song.all.length).include?(input)
